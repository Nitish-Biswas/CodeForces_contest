...
def solve():
    ...
    for i,cards in pairs:
        for prev in range(k-cards+1):
            for u in range(cards):
                prev_happ = dp[prev][u]
                ...
                diff = (cards - u) * (n - i)
                new_happ = prev_happ + diff
                
                new = prev + cards
                
                dp[new][cards] = max(dp[new][cards], new_happ)
